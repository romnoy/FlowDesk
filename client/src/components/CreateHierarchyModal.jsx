...
export default function CreateHierarchyModal({
  ...
  const [selectedAreaId, setSelectedAreaId] = useState(parentId || '');
  ...
  const areaSelectId = `${componentId}-area`;
  const errorId = `${componentId}-error`;
  const nameHelpId = `${componentId}-name-help`;
  const areaHelpId = `${componentId}-area-help`;

  const isArea = type === 'area';
  const isProject = type === 'project';
  const shouldShowAreaSelect = isProject && !parentId;
  ...
  const handleSubmit = async (e) => {
    ...
    if (shouldShowAreaSelect && !selectedAreaId) {
    ...
    if (isArea) {
      url = 'http://localhost:3001/api/hierarchy/areas';
      body = { name: trimmedName };
    } else if (isProject) {
      url = 'http://localhost:3001/api/hierarchy/projects';
      body = {
        name: trimmedName,
  ...
  const areaDescribedBy = [
    areaHelpId,
    error && errorField === 'area' ? errorId : null
  ]
    .filter(Boolean)
    .join(' ');
    ...
    >
      <div className="relative w-full max-w-md bg-white border border-dark-600 rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center mb-6">
          ...
          >
            {isArea ? 'יצירת קטגוריה חדשה' : 'יצירת פרויקט חדש'}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          {shouldShowAreaSelect && (
            <div className="space-y-1.5">
              ...
              <p id={areaHelpId} className="sr-only">
                יש לבחור קטגוריה שאליה הפרויקט ישויך.
              </p>
              ...
              <select
                ...
                className={`w-full bg-white border rounded-xl px-3 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-emerald-700 transition-colors disabled:opacity-70 disabled:cursor-wait ${errorField === 'area'
                    ? 'border-rose-700'
                    : 'border-dark-600'
                  }`}
                disabled={isLoading}
                aria-required="true"
                aria-invalid={errorField === 'area'}
